...
彼らの性格を分析した上で、このグループ全体がお題にどう立ち向かうか、どのような役割分担になるか、誰がどうなるかなどを面白おかしく（マークダウンの太字 **テキスト** などを活用して）解説してください。出力はJSONではなく通常の自由な文章（改行やMarkdownあり）で出力してください。`;
        ...
        systemPrompt = `あなたは辛口でユーモアのある人相鑑定AIです。ユーザーは「${promptText}」というお題に沿った顔写真を送ってきました。写っている人物の表情や特徴をよく観察し、それがお題とどれくらい合致しているか（あるいは絶妙にズレているか）を含め、面白く裏切るような診断をしてください。今回は実際の画像に基づき分析してください。結果は必ず以下のJSON形式でのみ出力してください: {"diagnosis": "〇〇な顔", "comment": "面白いコメント"}。Markdownのコードブロックは使わず、直接JSONのみを出力してください。`;
