...
出力はJSONではなく通常の自由な文章（改行あり）ですが、必ず**小見出し（### など）**を付けて読みやすい構成にしてください。`;
      ...
      if (typeof e.message === 'string' && e.message.includes('429')) {
        friendlyError = "AI利用の無料枠（時間あたりの制限）を超過しました。約1分ほど待ってから再度お試しください。";
      } else if (e.status === 429 || (e.response && e.response.status === 429)) {
        friendlyError = "AI利用制限を超過しました。少し待ってから再度お試しください。";
      }
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              roast_comment: "### おっと！\n少しばかりアクセスが集中しちまってるみたいだな！ちょっと待ってからもう一回試してくれよな！",
              comment: `エラーが発生しました。詳細: ${friendlyError}`,
              theme: '通信エラーコンビ',
              compatibility: 0,
              bestPair: '-',
              details: `### エラーが発生しました\n\n詳細: ${friendlyError}`
